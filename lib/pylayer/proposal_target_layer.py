...
def _sample_rois(all_rois, gt_boxes, rois_per_image, num_classes, gt_masks, im_scale, mask_info):
    ...
    fg_inds = []
    for i in xrange(len(cfg.TRAIN.FG_FRACTIONS)):
        cur_inds = np.where((max_overlaps >= cfg.TRAIN.FG_THRESH_LOS[i]) &
                            (max_overlaps <= cfg.TRAIN.FG_THRESH_HIS[i]))[0]
        cur_rois_this_image = min(cur_inds.size, np.round(rois_per_image *
                                                          cfg.TRAIN.FG_FRACTIONS[i]))
        if cur_inds.size > 0:
    ...
    bg_inds = []
    for i in xrange(len(cfg.TRAIN.BG_FRACTIONS)):
        cur_inds = np.where((max_overlaps >= cfg.TRAIN.BG_THRESH_LOS[i]) &
                            (max_overlaps <= cfg.TRAIN.BG_THRESH_HIS[i]))[0]
        cur_rois_this_image = min(cur_inds.size, np.round(bg_rois_per_this_image *
                                                          cfg.TRAIN.BG_FRACTIONS[i]))
        if cur_inds.size > 0:
            cur_inds = npr.choice(cur_inds, size=cur_rois_this_image, replace=False)
